...
async def chat_completions(
    ...
    messages, model, streaming, max_tokens, stream_options = (
        data.messages,
        data.model,
        data.stream,
        data.max_tokens,
    ...
    if (
        max_tokens
        and sum((max_tokens, prompt_tokens)) > app.state.models[model]["tokens"]
    ):
        raise HTTPException(
            detail={
                "error": {
...
if __name__ == "__main__":
    CommandLineInterface().run(["api:app", "--bind", "127.0.0.1", "--port", "9000"])


def start_server(tokens: list, address: str = "127.0.0.1", port: str = "9000"):
