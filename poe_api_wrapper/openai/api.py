...
async def call_tools(messages, tools, tool_choice):
    response = await message_handler(
        "gpt4_o_mini", messages, 128000, tools, tool_choice
    )
    tool_calls = None
    client, _ = await rotate_token(app.state.tokens)
    async for chunk in client.send_message(
        bot="gpt4_o_mini", message=response["message"]
    ):
        try:
            res_list = orjson.loads(
...
async def index() -> ORJSONResponse:
    return ORJSONResponse(
        {
            "message": "Welcome to Poe Api Wrapper reverse proxy!",
...
@app.api_route(
    "/models", methods=["GET", "POST", "PUT", "PATCH", "HEAD"], response_model=None
)
@app.api_route(
    "/v1/models", methods=["GET", "POST", "PUT", "PATCH", "HEAD"], response_model=None
)
        ...
        return ORJSONResponse(
            {
                ...
                "owned_by": app.state.models[model]["owned_by"],
                "tokens": app.state.models[model]["tokens"],
                "endpoints": app.state.models[model]["endpoints"],
            }
        )
    modelsData = [
        {
        ...
        for model, values in app.state.models.items()
    ]
...
async def chat_completions(
    request: Request, data: ChatData
) -> Union[StreamingResponse, ORJSONResponse]:
    messages, model, streaming, max_tokens, stream_options, tools, tool_choice = (
        data.messages,
        data.model,
        data.stream,
        data.max_tokens,
    ...
    if tools and len(tools) > 20:
        raise HTTPException(
            detail={
                "error": {
                    "message": "Maximum 20 tools are allowed.",
                    "type": "error",
                    "param": None,
                    "code": 400,
                }
    ...
    baseModel, tokensLimit, endpoints, premiumModel = (
        modelData["baseModel"],
        modelData["tokens"],
        modelData["endpoints"],
        modelData["premium_model"],
    )
    ...
    if premiumModel and not subscription:
        raise HTTPException(
            detail={
                "error": {
                    "message": "Premium model requires a subscription.",
                    "type": "error",
                    "param": None,
                    "code": 402,
                }
    ...
    prompt_tokens = await helpers.__tokenize(
        "".join([str(message) for message in response["message"]])
    )
    ...
    if prompt_tokens > tokensLimit:
        raise HTTPException(
            detail={
                "error": {
                    "message": f"Your prompt exceeds the maximum context length of {tokensLimit} tokens.",
                    "type": "error",
                    "param": None,
                    "code": 400,
                }
    ...
    if raw_tool_calls:
        ...
        prompt_tokens = await helpers.__tokenize(
            "".join([str(message["content"]) for message in text_messages])
        )
    ...
    return (
        ...
        if streaming
        else await non_streaming_response(
            client,
            response,
            model,
            completion_id,
...
@app.api_route(
    "/v1/images/generations", methods=["POST", "OPTIONS"], response_model=None
)
async def create_images(request: Request, data: ImagesGenData) -> ORJSONResponse:
    ...
    if size == "1024x1024":
        ...
        "sizes" in app.state.models[model] and size in app.state.models[model]["sizes"]
    ):
        ...
        raise HTTPException(
            detail={
                "error": {
                    "message": f"Invalid size for {model}. Available sizes: {', '.join(app.state.models[model]['sizes']) if 'sizes' in app.state.models[model] else '1024x1024'}",
                    "type": "error",
                    "param": None,
                    "code": 400,
                }
    ...
    for _ in range(n):
        ...
        urls.extend(
            [url for url in image_generation.split() if url.startswith("https://")]
        )
        if len(urls) >= n:
            break
    urls = urls[-n:]
    ...
    if len(urls) == 0:
        raise HTTPException(
            detail={
                "error": {
                    "message": f"The provider for {model} sent an invalid response.",
                    "type": "error",
                    "param": None,
                    "code": 500,
                }
        ...
        for url in urls:
            ...
            if not content_type.startswith("image/"):
                raise HTTPException(
                    detail={
                        "error": {
                            "message": "The content returned was not an image.",
                            "type": "error",
                            "param": None,
                            "code": 500,
                        }
...
async def edit_images(request: Request, data: ImagesEditData) -> ORJSONResponse:
    ...
    if not (
        isinstance(image, str) and (os.path.exists(image) or image.startswith("http"))
    ):
        raise HTTPException(
            detail={
                "error": {
    ...
    if "/v1/images/edits" not in endpoints:
        raise HTTPException(
            detail={
                "error": {
                    "message": "This model does not support image editing.",
                    "type": "error",
                    "param": None,
                    "code": 400,
                }
...
async def image_handler(baseModel: str, prompt: str, tokensLimit: int) -> dict:
    try:
        message = await helpers.__progressive_summarize_text(
            prompt, min(len(prompt), tokensLimit)
        )
        ...
        raise HTTPException(
            detail={
                "error": {
                    "message": f"Failed to truncate prompt. Error: {e}",
                    "type": "error",
                    "param": None,
                    "code": 400,
                }
    ...
    try:
        ...
        if full_tokens > tokensLimit:
            history_string = await helpers.__progressive_summarize_text(
                history_string,
                tokensLimit - await helpers.__tokenize(main_request) - 100,
            )
        ...
        raise HTTPException(
            detail={
                "error": {
                    "message": f"Failed to process messages. Error: {e}",
                    "type": "error",
                    "param": None,
                    "code": 400,
                }
...
async def generate_image(
    client: AsyncPoeApi, response: dict, aspect_ratio: str, image: list = []
) -> str:
    try:
        async for chunk in client.send_message(
            bot=response["bot"],
        ...
        raise HTTPException(
            detail={
                "error": {
                    "message": f"Failed to generate image. Error: {e}",
                    "type": "error",
                    "param": None,
                    "code": 500,
                }
                        ...
                        [
                            ChoiceDeltaToolCall(
                                ...
                                function=ChoiceDeltaToolCallFunction(
                                    name=tool_call["name"],
                                    arguments=orjson.dumps(tool_call["arguments"]),
                                ),
                            )
...
async def generate_chunks(
    ...
    try:
        ...
        if not raw_tool_calls:
            async for chunk in client.send_message(
                bot=response["bot"], message=response["message"], file_path=image_urls
            ):
            ...
            chunk_token = await helpers.__tokenize(
                "".join(
                    [
                        str(tool_call["name"]) + str(tool_call["arguments"])
                        for tool_call in raw_tool_calls
                    ]
                )
            )
        ...
        raise HTTPException(
            detail={
                "error": {
                    "message": f"Failed to stream response. Error: {e}",
                    "type": "error",
                    "param": None,
                    "code": 500,
                }
    ...
    if not raw_tool_calls:
        try:
            ...
            raise HTTPException(
                detail={
                    "error": {
                        "message": f"Failed to generate completion. Error: {e}",
                        "type": "error",
                        "param": None,
                        "code": 500,
                    }
        ...
        choices=[
            ChatCompletionResponseChoice(
                ...
                message=MessageResponse(
                    role="assistant",
                    content=None if raw_tool_calls else chunk["text"],
                    tool_calls=(
                        [
...
async def rotate_token(tokens) -> Tuple[AsyncPoeApi, bool]:
    if len(tokens) == 0:
        raise HTTPException(
            detail={
                "error": {
                    "message": "All tokens have been used. Please add more tokens.",
                    "type": "error",
                    "param": None,
                    "code": 402,
                }
...
if __name__ == "__main__":
    CommandLineInterface().run(["api:app", "--bind", "0.0.0.0", "--port", "9000"])


def start_server(tokens: list, address: str = "0.0.0.0", port: str = "9000"):
    ...
    CommandLineInterface().run(
        ["poe_api_wrapper.openai.api:app", "--bind", f"{address}", "--port", f"{port}"]
    )
